...
% etc. to indicate your MS, PhD, or BS students as indicated by the template
...
\newcommand{\UNLrank}{Assistant Professor}
\newcommand{\UNLdept}{Computer Science and Engineering Department}
...
% NOTE 1:
% For \pubaddendum[tier][h-index or impact]:
% Parameters:
% #1 (optional) = what tier (first, second, third)
% #2 (optional) = other information (h-index, impact factor, etc.)
% Usage examples:
% \pubaddendum[1] yields "(1st tier)" in bold
% \pubaddendum[2][25\% acceptance rate] yields "(2nd tier, 25% acceptance rate)" in bold
% \pubaddendum[][impact: 5.138] yields "(impact: 5.138)" in bold

% NOTE 2:
% For the publications, I use J# (e.g., J1, J2) for journals, and C# (e.g., C1,
...
% rather than journalbib or conferencebib.
...
% publications. To get the bold author you'll need to modify the
% bibliographystyle file. I can share mine if you want.
...
% \begin{mybibenum}
\begin{journalbib}
  % \setcounter{journalbibi}{1}
\item \bibentry{shang2020cooptimal}, \pubaddendum[][impact: 4.292]
\end{journalbib}
...
% restart the mybibenum counter
% \restartlist{mybibenum}
\begin{conferencebib}
\item \bibentry{balasubramaniam2020investigating}, \pubaddendum[1][24\%
  acceptance rate], \textbf{\emph{ICCPS Best Paper session}}
\end{conferencebib}
